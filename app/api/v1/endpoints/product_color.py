...
# GET: Fetch all data
@router.get("/product_color", response_model=List[ProductColorOut])
async def get_all(db : Session = Depends(get_database)):
    return product_color_crud.get_all(db=db)

# GET: Fetch with id
@router.get("/product_color/{product_id}{color_id}", response_model=ProductColorOut)
...
# POST: Create new row
@router.post("/product_color", response_model=ProductColorOut)
...
# PUT: Update row
@router.put("/product_color/{product_id}{color_id}", response_model=ProductColorOut)
...
# DELETE: Delete row
@router.delete("/product_color/{product_id}{color_id}", response_model=ProductColorOut)
